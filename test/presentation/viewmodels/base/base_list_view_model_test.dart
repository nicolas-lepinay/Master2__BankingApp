...
class TestItem {
  ...
  const TestItem({required this.id, required this.name, required this.date});
}
...
class TestListViewModel extends BaseListViewModel<TestListViewState, TestItem> {
  ...
  TestListViewState updateDateFilterState(DateRange? dateFilter) {
    return (state).copyWith(dateFilter: dateFilter);
  }
  ...
  TestListViewState clearFiltersState() {
    return const TestListViewState(
      ...
      error: null,
      dateFilter: null,
    ).copyWith(items: state.items, filteredItems: state.items);
  }
}
  ...
  group('BaseListViewModel', () {
    ...
    group('Search Functionality', () {
      ...
      test('should reset pagination when search changes', () async {
        ...
        viewModel.goToPage(
          1,
        ); // Va à la page 1 (qui devrait contenir "Cherry", "Date")
    ...
    group('Date Filtering', () {
      ...
      test('should clear all filters', () async {
        await viewModel.updateSearchQuery('apple');
        await viewModel.updateDateFilter(
          DateRange(start: DateTime(2024, 1, 1), end: DateTime(2024, 1, 5)),
        );
