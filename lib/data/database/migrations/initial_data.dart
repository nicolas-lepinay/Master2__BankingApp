...
Future<void> insertInitialData(AppDatabase database) async {
  ...
  final netflixTransactionId = await database.into(database.transactions).insert(
  ...
  final spotifyTransactionId = await database.into(database.transactions).insert(
    TransactionsCompanion(
      accountId: const Value(1),
  ...
  final refundTransactionId = await database.into(database.transactions).insert(
    TransactionsCompanion(
      accountId: const Value(1),
  ...
  await database.into(database.followedTransactions).insert(
