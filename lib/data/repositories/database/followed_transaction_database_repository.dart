...
class FollowedTransactionDatabaseRepository {
  ...
  Future<void> addFollowedTransaction(int transactionId) async {
    ...
    final existingFollow = await (_database.select(_database.followedTransactions)
          ..where((ft) => ft.transactionId.equals(transactionId)))
        .getSingleOrNull();
    ...
    await _database.into(_database.followedTransactions).insert(
  ...
  Future<void> removeFollowedTransaction(int transactionId) async {
    final deletedRows = await (_database.delete(_database.followedTransactions)
          ..where((ft) => ft.transactionId.equals(transactionId)))
        .go();
  ...
  Future<bool> isTransactionFollowed(int transactionId) async {
    final followedTransaction = await (_database.select(_database.followedTransactions)
          ..where((ft) => ft.transactionId.equals(transactionId)))
        .getSingleOrNull();
  ...
  Future<List<TransactionWithCounterparty>> getFollowedTransactionsWithDetails() async {
    final query = _database.select(_database.followedTransactions).join([
      leftOuterJoin(
        _database.transactions,
        _database.transactions.id.equalsExp(_database.followedTransactions.transactionId),
      ),
      leftOuterJoin(
  ...
  Future<List<int>> getFollowedTransactionIds() async {
    final results = await _database.select(_database.followedTransactions).get();
  ...
  Future<List<FollowedTransaction>> getFollowedTransactions() async {
    return await (_database.select(_database.followedTransactions)
