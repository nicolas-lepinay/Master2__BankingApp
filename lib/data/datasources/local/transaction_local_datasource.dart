...
class TransactionLocalDataSourceImpl implements TransactionLocalDataSource {
  ...
  Future<List<TransactionModel>> getAllTransactions() async {
    ...
    return transactions.map((transaction) => TransactionModel.fromDrift(transaction)).toList();
  }
  ...
  Future<List<TransactionModel>> getTransactionsByCategory(
    ...
    final transactions = await (_database.select(_database.transactions)
      ..where((tbl) => tbl.accountId.equals(accountId))
      ..where((tbl) => tbl.category1Id.equals(categoryId) |
  ...
  Stream<List<TransactionModel>> watchAllTransactions() {
    return _database.select(_database.transactions).watch().map(
      (transactions) => transactions.map((transaction) => TransactionModel.fromDrift(transaction)).toList()
    );
  }
