...
class SearchFilters {
  ...
  @override
  String toString() => 'SearchFilters(keyword: "$keyword", minAmount: $minAmount, maxAmount: $maxAmount, categoryId: $categoryId, counterpartyId: $counterpartyId, startDate: $startDate, endDate: $endDate, activeType: $activeType)';
}

/// État pour la recherche
class SearchViewState extends BaseViewState {
  ...
  SearchViewState failure(String errorMessage) {
  ...
  @override
  String toString() => 'SearchViewState(allTransactions: ${allTransactions.length}, searchResults: ${searchResults.length}, filters: $filters, isLoading: $isLoading, error: $error)';
}

/// ViewModel pour la recherche
class SearchViewModel extends BaseViewModel<SearchViewState> 
    with ListViewModelMixin<SearchViewState, domain.TransactionWithBalance>,
         PaginationViewModelMixin<SearchViewState, domain.TransactionWithBalance> {
  ...
  Future<void> _loadMetadata() async {
    await executeWithErrorHandling(() async {
      ...
      final counterparties = await _counterpartyRepository.getAllCounterparties();
  ...
  Future<void> _performSearch() async {
    var results = List<domain.TransactionWithBalance>.from(state.allTransactions);
    ...
    if (state.filters.minAmount != null || state.filters.maxAmount != null) {
      results = results
          .where((tx) => tx.isInAmountRange(state.filters.minAmount, state.filters.maxAmount))
          .toList();
    }
    ...
    if (state.filters.categoryId != null) {
      results = results
          .where((tx) => tx.transaction.categoryIds.contains(state.filters.categoryId))
          .toList();
    }
    ...
    if (state.filters.counterpartyId != null) {
      results = results
          .where((tx) => tx.transaction.counterpartyId == state.filters.counterpartyId)
          .toList();
    }
    ...
    if (state.filters.startDate != null || state.filters.endDate != null) {
      results = results.where((tx) {
        ...
        if (state.filters.startDate != null && date.isBefore(state.filters.startDate!)) {
          return false;
        }
        if (state.filters.endDate != null && date.isAfter(state.filters.endDate!)) {
          return false;
        }
        return true;
      }).toList();
  ...
  domain.Counterparty? getCounterpartyById(int id) {
    try {
      return state.counterparties.firstWhere((counterparty) => counterparty.id == id);
    } catch (e) {
      return null;
    }
  }
  ...
  List<String> getSearchSuggestions(String query) {
    ...
    for (final tx in state.allTransactions) {
      ...
      if (title.contains(lowerQuery) && !suggestions.contains(tx.displayTitle)) {
    ...
    for (final counterparty in state.counterparties) {
      ...
      if (name.contains(lowerQuery) && !suggestions.contains(counterparty.name)) {
  ...
  Map<String, dynamic> getSearchStats() {
    final incomeTransactions = state.searchResults.where((tx) => tx.isIncome).toList();
    final expenseTransactions = state.searchResults.where((tx) => tx.isExpense).toList();
    
    return {
      'totalTransactions': state.searchResults.length,
      'incomeTransactions': incomeTransactions.length,
      'expenseTransactions': expenseTransactions.length,
      'totalIncome': incomeTransactions.fold(0.0, (sum, tx) => sum + tx.transaction.amount),
      'totalExpenses': expenseTransactions.fold(0.0, (sum, tx) => sum + tx.transaction.amount),
      'averageAmount': state.searchResults.isEmpty 
          ? 0.0 
          : state.searchResults.fold(0.0, (sum, tx) => sum + tx.transaction.amount) / state.searchResults.length,
