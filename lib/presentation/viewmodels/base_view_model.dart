...
abstract class BaseViewModel<T extends BaseViewState> extends StateNotifier<T> {
  ...
  String? get errorMessage => state is ErrorState ? (state as ErrorState).message : null;
...
mixin ListViewModelMixin<T extends BaseViewState, Item> on BaseViewModel<T> {
  ...
  List<Item> searchItems(List<Item> items, String query, String Function(Item) getText) {
...
mixin PaginationViewModelMixin<T extends BaseViewState, Item> on BaseViewModel<T> {
  int _currentPage = 0;
